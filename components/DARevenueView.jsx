/**
 * components/DARevenueView.jsx — DA 광고 매출 상세 (브랜드 공용)
 */
...
export default function DARevenueView({ brand }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/revenue-detail?brand=${brand.id}&type=da&startDate=${start}&endDate=${end}`);
  ...
  return (
    <div className="min-h-screen bg-slate-50" style={{fontFamily:"'Pretendard','Apple SD Gothic Neo',sans-serif"}}>
      <Head>
        <title>{`DA 광고 매출 상세 | ${brand.name}`}</title>
      ...
      <main className="max-w-screen-xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="rounded-2xl border p-5 shadow-sm md:col-span-2" style={{background:brand.colorSoft,borderColor:BC+'33'}}>
            <p className="text-sm font-semibold mb-2" style={{color:BC}}>📣 DA 광고 매출 합계</p>
                ...
                {chartType === 'line' && (
                  <ResponsiveContainer width="100%" height={320}>
                    <LineChart data={chartData}>
                      ...
                      <Line type="monotone" dataKey="매출액" stroke={BC} strokeWidth={2.5} dot={false} activeDot={{r:5}}/>
                    </LineChart>
                  </ResponsiveContainer>
                )}
                {chartType === 'bar' && (
                  <ResponsiveContainer width="100%" height={320}>
                    <BarChart data={chartData}>
                      ...
                      <Bar dataKey="매출액" fill={BC} radius={[4,4,0,0]}/>
                    </BarChart>
                  </ResponsiveContainer>
                )}
                {chartType === 'pie' && (
              ...
              <table className="w-full text-sm">
                <thead>
                  ...
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    {(data?.weeks || []).map(w => (
                      <Fragment key={w}>
                        <th className="px-3 py-2 text-right text-xs text-gray-400">광고비</th>
                        <th className="px-3 py-2 text-right text-xs text-gray-400">매출액</th>
                        <th className="px-3 py-2 text-right text-xs text-gray-400">ROAS</th>
                      </Fragment>
                    ))}
                  </tr>
                </thead>
                  ...
                  {data.weeklyTable.map((row, i) => (
                    <tr key={i} className="hover:bg-gray-50">
                      ...
                      {(data?.weeks || []).map(w => {
                        ...
                        return (
                          <Fragment key={w}>
                            <td className="px-3 py-3 text-right text-gray-600 whitespace-nowrap">{week ? krw(week.spend) : '-'}</td>
                            <td className="px-3 py-3 text-right text-gray-700 font-semibold whitespace-nowrap">{week ? krw(week.revenue) : '-'}</td>
                            <td className={`px-3 py-3 text-right whitespace-nowrap ${rCls}`}>{week && roas > 0 ? `${roas}%` : '-'}</td>
                          </Fragment>
                        );
                      })}
                    </tr>
