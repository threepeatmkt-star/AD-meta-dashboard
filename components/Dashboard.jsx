/**
 * components/Dashboard.jsx — 브랜드 공용 메인 대시보드
 * 이 파일 하나만 고치면 삼대오백/엠엑시브 양쪽에 모두 반영됩니다.
 */
import { useState, useEffect, useCallback, useRef } from 'react';
import { useRouter } from 'next/router';
import Head from 'next/head';
import { extractProduct, normalizeProduct, campaignOrder } from '../lib/brands';
...
function groupByProduct(rows, brand) {
  const map={};
  rows.forEach(r=>{
    const prod=normalizeProduct(extractProduct(r.name), brand),key=`${r.campaignName}||${prod}`;
...
function Card({label,value,curr,prev,isRoas,roasVal,loading,accent,sub,deltaType='pct',brand}) {
  return (
    <div className={`rounded-2xl border p-5 shadow-sm ${accent?'border-transparent':'bg-white border-gray-200'}`}
      style={accent?{background:brand.colorSoft,borderColor:brand.color+'33'}:{}}>
      <p className="text-sm font-semibold mb-2" style={accent?{color:brand.color}:{color:'#9CA3AF'}}>{label}</p>
      {loading?<div className="h-8 bg-gray-100 rounded animate-pulse w-28"/>
        :<>
          <div className="flex items-baseline flex-wrap gap-1">
            <span className={`text-2xl font-bold ${isRoas?roasCls(roasVal):'text-gray-900'}`}
              style={!isRoas&&accent?{color:brand.color}:{}}>{value}</span>
...
function InsightPanel({data,level,dateRange,brand}) {
...
function exportToCSV(data,tab,dateRange,brand) {
  ...
  a.download=`${brand.name}_광고리포트_${dateRange.replace(/~/g,'-').replace(/\s/g,'')}_${tab}.csv`;
...
export default function Dashboard({ brand }) {
  ...
  const load=useCallback(async()=>{
    ...
    try {
      const r=await fetch(`/api/dashboard?brand=${brand.id}&startDate=${start}&endDate=${end}&level=${tab}`);
      const j=await r.json();
      if(j.error) throw new Error(j.error);
      ...
      if(compare){
        ...
        const pr=await fetch(`/api/dashboard?brand=${brand.id}&startDate=${ps}&endDate=${pe}&level=${tab}`);
  ...
  const copyOverview = async () => {
    setCopying(true);
    try { await captureAndCopy(overviewRef, `${brand.name}_개요_${dateRangeLabel.replace(/\s/g,'')}.png`); }
    catch(e) { alert('캡처 실패: ' + e.message); }
    finally { setCopying(false); }
  };
  const copyAdsetTable = async () => {
    setAdsetCopying(true);
    try { await captureAndCopy(adsetTableRef, `${brand.name}_광고세트_${adsetSubTab==='product'?'제품별':'세트별'}_${dateRangeLabel.replace(/\s/g,'')}.png`); }
    catch(e) { alert('캡처 실패: ' + e.message); }
          ...
          <div className="flex flex-wrap items-center gap-2">
            {PRESETS.map(p=>(
              <button key={p.v} onClick={()=>setPreset(p.v)}
                ...
                style={preset===p.v?{background:BC}:{}}>
                {p.l}
              </button>
            ))}
            {preset==='custom'&&(
      ...
      <main className="max-w-screen-xl mx-auto px-6 py-6 space-y-6">
        ...
        <div ref={overviewRef} className="rounded-2xl p-1">
          <div className="flex items-center gap-2 mb-3 px-1">
            <span className="text-sm font-bold" style={{color:BC}}>{brand.name}</span>
          ...
          <div className="grid grid-cols-3 gap-4">
            <Card brand={brand} label="광고비 (Meta)" value={krw(totals.spend)} curr={totals.spend} prev={compare?prevTotals.spend:undefined} loading={loading} deltaType="currency"/>
            <Card brand={brand} label="총 매출 (GA4·SNS)" value={krw(adRevenue+viralRevenue)} curr={adRevenue+viralRevenue} prev={compare?prevAdRevenue+prevViralRevenue:undefined} loading={loading} deltaType="currency"/>
            <Card brand={brand} label="ROAS" value={`${totals.roas}%`} curr={totals.roas} prev={compare?prevTotals.roas:undefined} isRoas roasVal={totals.roas} loading={loading} deltaType="pct"/>
          </div>
          <div className="grid grid-cols-4 gap-4 mt-4">
            <Card brand={brand} label="도달" value={num(totals.reach)} curr={totals.reach} prev={compare?prevTotals.reach:undefined} loading={loading} deltaType="number"/>
            <Card brand={brand} label="노출" value={num(totals.impressions)} curr={totals.impressions} prev={compare?prevTotals.impressions:undefined} loading={loading} deltaType="number"/>
            <Card brand={brand} label="클릭 수" value={num(totals.clicks)} curr={totals.clicks} prev={compare?prevTotals.clicks:undefined} loading={loading} deltaType="number"/>
            <Card brand={brand} label="클릭률 (CTR)" value={ctr(totals.clicks,totals.impressions)} curr={totals.impressions>0?totals.clicks/totals.impressions*100:0} prev={compare&&prevTotals.impressions>0?prevTotals.clicks/prevTotals.impressions*100:undefined} loading={loading} sub="클릭 ÷ 노출" deltaType="pct"/>
          </div>
          <div className="mt-4">
            <SectionLabel>자사몰 (GA4) 매출 개요 — SNS 채널 기준 (fb / insta / ig)</SectionLabel>
            <div className="grid grid-cols-2 gap-4">
              <div onClick={()=>router.push(`/${brand.id}/da-revenue?start=${rangeStart}&end=${rangeEnd}`)} className="cursor-pointer hover:shadow-md transition-shadow">
                <Card brand={brand} label="📣 DA 광고 매출  →" value={krw(adRevenue)} curr={adRevenue} prev={compare?prevAdRevenue:undefined} loading={loading} accent sub="클릭하면 상세 보기" deltaType="currency"/>
              </div>
              <div onClick={()=>router.push(`/${brand.id}/viral-revenue?start=${rangeStart}&end=${rangeEnd}`)} className="cursor-pointer hover:shadow-md transition-shadow">
                <Card brand={brand} label="📱 SNS 바이럴 매출  →" value={krw(viralRevenue)} curr={viralRevenue} prev={compare?prevViralRevenue:undefined} loading={loading} accent sub="클릭하면 상세 보기" deltaType="currency"/>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center border-b border-gray-100 overflow-x-auto">
            {TABS.map(t=>(
              <button key={t.v} onClick={()=>{setTab(t.v);setSelectedAdsets([]);}}
                className={`px-7 py-4 text-base font-bold border-b-2 whitespace-nowrap transition-colors ${tab===t.v?'bg-gray-50':'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}
                style={tab===t.v?{borderColor:BC,color:BC}:{}}>
                {t.l}
              </button>
            ))}
            <div className="ml-auto px-4 flex items-center gap-3">
              <button onClick={()=>exportToCSV(displayData,tab,dateRangeLabel,brand)} disabled={!displayData.length}
              ...
              <div className="flex">
                {[{v:'adset',l:'세트별'},{v:'product',l:'캠페인 · 제품별'}].map(st=>(
                  <button key={st.v} onClick={()=>setAdsetSubTab(st.v)}
                    className={`px-6 py-3 text-sm font-bold transition-colors ${adsetSubTab===st.v?'border-b-2 bg-white':'text-gray-400 hover:text-gray-600'}`}
                    style={adsetSubTab===st.v?{color:BC,borderColor:BC}:{}}>
                    {st.l}
                  </button>
                ))}
              </div>
            ...
            <div className="overflow-x-auto" ref={tab==='adset'?adsetTableRef:null}>
              {tab==='adset'&&(
                <div className="flex items-center gap-2 px-5 pt-3 pb-1">
                  <span className="text-xs font-bold" style={{color:BC}}>{brand.name}</span>
              ...
              <table className="w-full text-base">
                            ...
                            <div>
                              ...
                              {tab==='ad'&&row.adsetName&&(
                                <button onClick={()=>setSelectedAdsets([row.adsetName])}
                                  className="text-sm hover:underline mt-0.5 text-left truncate max-w-xs block font-medium" style={{color:BC}}>
                                  {row.adsetName}
                                </button>
                              )}
                            </div>
                ...
                {displayData.length>0&&(
                  <tfoot>
                    <tr className="border-t-2" style={{background:brand.colorSoft,borderColor:BC+'55'}}>
                      {tab==='ad'&&<td className="px-5 py-4"/>}
                      <td className="px-5 py-4 text-base font-bold" style={{color:BC}}>합계 / 전체</td>
                      {tab!=='campaign'&&adsetSubTab!=='product'&&<td className="px-5 py-4"/>}
                      {tab==='adset'&&<td className="px-5 py-4"/>}
                      <td className="px-5 py-4 text-right text-base font-bold whitespace-nowrap" style={{color:BC}}>{krw(dispTotals.spend)}</td>
                      <td className="px-5 py-4 text-right text-base font-bold whitespace-nowrap" style={{color:BC}}>{krw(dispTotals.revenue)}</td>
                      <td className={`px-5 py-4 text-right text-base whitespace-nowrap ${roasCls(dispTotals.roas)}`}>{dispTotals.roas}%</td>
                      <td className="px-5 py-4 text-right text-base font-bold" style={{color:BC}}>{num(dispTotals.reach)}</td>
                      <td className="px-5 py-4 text-right text-base font-bold" style={{color:BC}}>{num(dispTotals.impressions)}</td>
                      <td className="px-5 py-4 text-right text-base font-bold" style={{color:BC}}>{num(dispTotals.clicks)}</td>
                      <td className="px-5 py-4 text-right text-base font-bold" style={{color:BC}}>{ctr(dispTotals.clicks,dispTotals.impressions)}</td>
                    </tr>
                  </tfoot>
                )}
              </table>
        ...
        <InsightPanel data={displayData} level={tab} dateRange={dateRangeLabel} brand={brand}/>
