...
function getPresetRange(preset) {
  ...
  const dow = now.getDay(), diffMon = dow===0?6:dow-1;
  const thisMon = new Date(now-diffMon*86400000);
...
function getPrevRange(start, end) {
  const s=new Date(start),e=new Date(end),days=Math.ceil((e-s)/86400000)+1;
  const pe=new Date(s-86400000),ps=new Date(pe-(days-1)*86400000);
...
function Spinner() {
  return <svg className="w-6 h-6 animate-spin text-blue-500" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>;
}

// ── 카드 ──────────────────────────────────────────────────────────
function MetricCard({label, value, curr, prev, highlight, isRoas, roasVal, loading, sub}) {
  return (
    <div className={`rounded-2xl border p-5 shadow-sm ${highlight ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200'}`}>
      <p className={`text-sm font-semibold mb-2 ${highlight ? 'text-blue-500' : 'text-gray-400'}`}>{label}</p>
      {loading
        ? <div className="h-8 bg-gray-100 rounded animate-pulse w-28"/>
        : <>
            <div className="flex items-baseline flex-wrap gap-1">
              <span className={`text-2xl font-bold ${isRoas ? roasCls(roasVal) : highlight ? 'text-blue-700' : 'text-gray-900'}`}>{value}</span>
...
function SectionLabel({children}) {
  return (
    <div className="flex items-center gap-2 mb-3">
      <span className="text-sm font-bold text-gray-500 uppercase tracking-widest">{children}</span>
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50" style={{fontFamily:"'Pretendard','Apple SD Gothic Neo',sans-serif"}}>
      ...
      {/* 썸네일 호버 오버레이 */}
      {hoverThumb && (
        <div className="fixed z-[9999] pointer-events-none" style={{top:Math.max(8,hoverThumb.y-240),left:hoverThumb.x+24}}>
          <img src={hoverThumb.src} className="w-60 h-60 object-cover rounded-2xl shadow-2xl border-2 border-white"/>
        </div>
      )}
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-20 shadow-sm">
        <div className="max-w-screen-xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            {/* Meta 스타일 로고 */}
            <div className="w-12 h-12 rounded-xl flex items-center justify-center shadow" style={{background:'linear-gradient(135deg,#1877F2,#0a5ec0)'}}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="white"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14H9V10h2v6zm4 0h-2v-3.5c0-.83-.67-1.5-1.5-1.5S10 11.67 10 12.5V16H8v-6h2v.93c.5-.81 1.33-1.43 2.25-1.43C13.77 9.5 15 10.73 15 12.25V16z"/></svg>
            </div>
            <div>
              <div className="flex items-center gap-2">
                <p className="text-lg font-bold text-gray-900 leading-tight">쓰리핏 메타광고 대시보드</p>
                <span className="px-2.5 py-0.5 rounded-full text-xs font-bold text-white" style={{background:'#1877F2'}}>삼대오백</span>
              </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {PRESETS.map(p=>(
              <button key={p.v} onClick={()=>setPreset(p.v)}
                ...
                style={preset===p.v?{background:'#1877F2'}:{}}>
                {p.l}
              </button>
            ))}
            {preset==='custom' && (
            ...
            <div className="ml-auto flex items-center gap-3 shrink-0">
              ...
              <button onClick={()=>setCompare(!compare)}
                ...
                style={compare?{background:'#1877F2'}:{}}>
      ...
      <main className="max-w-screen-xl mx-auto px-6 py-6 space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-4">
            <MetricCard label="광고비 (Meta)" value={krw(totals.spend)} curr={totals.spend} prev={compare?prevTotals.spend:undefined} loading={loading}/>
            <MetricCard label="총 매출 (GA4)" value={krw(totals.revenue)} curr={totals.revenue} prev={compare?prevTotals.revenue:undefined} loading={loading}/>
            <MetricCard label="ROAS" value={`${totals.roas}%`} curr={totals.roas} prev={compare?prevTotals.roas:undefined} isRoas roasVal={totals.roas} loading={loading}/>
            <MetricCard label="도달" value={num(totals.reach)} curr={totals.reach} prev={compare?prevTotals.reach:undefined} loading={loading}/>
            <MetricCard label="노출" value={num(totals.impressions)} curr={totals.impressions} prev={compare?prevTotals.impressions:undefined} loading={loading}/>
            <MetricCard label="클릭 수" value={num(totals.clicks)} curr={totals.clicks} prev={compare?prevTotals.clicks:undefined} loading={loading}/>
            <MetricCard label="클릭률 (CTR)" value={ctr(totals.clicks,totals.impressions)} curr={totals.impressions>0?totals.clicks/totals.impressions:0} prev={compare&&prevTotals.impressions>0?prevTotals.clicks/prevTotals.impressions:undefined} loading={loading} sub="클릭/노출"/>
          </div>
        </div>

        {/* ── 자사몰 GA4 매출 개요 ── */}
        <div>
          <SectionLabel>자사몰 (GA4) 매출 개요</SectionLabel>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <MetricCard label="📣 DA 광고 매출" value={krw(adRevenue)} curr={adRevenue} prev={compare?prevAdRevenue:undefined} loading={loading} highlight/>
            <MetricCard label="📱 SNS 바이럴 매출" value={krw(viralRevenue)} curr={viralRevenue} prev={compare?prevViralRevenue:undefined} loading={loading} highlight sub="소스/매체 cpm 미포함 기준"/>
          </div>
        </div>
            ...
            {tab==='ad'&&adsetOptions.length>0&&(
              <div className="ml-3 flex items-center gap-2">
                <select value={adsetFilter} onChange={e=>setAdsetFilter(e.target.value)}
                  className="border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-300 bg-white max-w-[240px] font-medium">
                    ...
                    return (
                      <tr key={i} className="hover:bg-blue-50/20 transition-colors">
                        ...
                        {tab!=='campaign'&&adsetSubTab!=='product'&&(
                          <td className="px-5 py-4"><p className="text-sm text-gray-400 truncate max-w-[160px] font-medium" title={row.campaignName}>{row.campaignName}</p></td>
                        )}
                        {tab==='adset'&&(
                          <td className="px-5 py-4 text-right whitespace-nowrap">
                            {row.dailyBudget>0?<span className="text-gray-700 font-semibold">{krw(row.dailyBudget)}</span>:<span className="text-sm text-gray-300 font-medium">CBO</span>}
                          </td>
                        )}
        ...
        <div className="flex items-center gap-5 text-sm text-gray-400 pb-3">
          ...
          <span className="ml-auto text-gray-300 text-xs">DA광고매출 = cpm 포함 | 바이럴매출 = cpm 미포함</span>
        </div>
      </main>
    </div>
  );
