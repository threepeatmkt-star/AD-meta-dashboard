/**
 * pages/viral-revenue.js — SNS 바이럴 매출 상세 페이지
 */
...
export default function ViralRevenuePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/revenue-detail?type=viral&startDate=${start}&endDate=${end}`);
      ...
      <main className="max-w-screen-xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 shadow-sm">
            <p className="text-sm font-semibold text-blue-500 mb-2">📱 바이럴 매출 합계</p>
                ...
                {chartType === 'line' && (
                  <ResponsiveContainer width="100%" height={320}>
                    <LineChart data={lineData}>
                      ...
                      <Legend/>
                      <Line yAxisId="left" type="monotone" dataKey="매출액" stroke={BLU} strokeWidth={2.5} dot={false}/>
                ...
                {chartType === 'bar' && (
                  <ResponsiveContainer width="100%" height={320}>
                    <BarChart data={barData}>
                      ...
                      <Tooltip formatter={(v,n) => n==='매출액' ? krw(v) : v.toLocaleString()+'건'}/>
                      <Legend/>
                      <Bar yAxisId="left" dataKey="매출액" fill={BLU} radius={[4,4,0,0]}/>
